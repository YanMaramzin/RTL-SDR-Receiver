...
RingBuffer::RingBuffer(uint32_t bufferSize): m_bufferSize(bufferSize), m_writePosition(0), m_readPosition(0),
                                             m_pushDataSize(0), m_popDataSize(0), m_buffer(bufferSize) {}
...
uint32_t RingBuffer::availableSpaceSize() const {
    return m_bufferSize - availableDataSize();
}
